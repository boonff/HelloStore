            ...
            skuList.forEach((item) => {
                (item.specInfo || []).forEach((subItem) => {
                    if (subItem.specValueId === specValueId && item.stockInfo.stockQuantity > 0) {
                        console.log("checkSkuStockQuantity: ", item.stockInfo.stockQuantity)
                        const subArray = [];
        ...
        chooseSpecValueId(specValueId, specId) {
            ...
            specList.forEach((item) => {
                if (item.specId === specId) {
                    const subSpecValueItem = item.values.find((subItem) => subItem.specValueId === specValueId);
